...
mod tests {
    ...
    fn authentication() {
        ...
        let authentication_token = token::Authentication::new(&issuer, 10, 11, &content);

        let encoded_authentication_token = authentication_token.to_vec();
        let decoded_authentication_token = match token::Token::decode(&encoded_authentication_token)
            .expect("failed to decode token")
        {
            token::Token::Authentication(decoded_authentication_token) => {
        ...
        assert_eq!(decoded_authentication_token.expires(), 11);
        assert_eq!(decoded_authentication_token.content_hash(), content_hash);
    }

    #[test]
    fn push() {
        let issuer = signing::KeyPair::new();
        let bearer = signing::KeyPair::new();
        let encrypted = exchange::KeyPair::new();
        let exchanged = exchange::KeyPair::new();
        let credential = b"secret push token";
        ...
        let encoded_push_token = push_token.to_vec();
        let decoded_push_token =
            match token::Token::decode(&encoded_push_token).expect("failed to decode token") {
    ...
    fn send() {
        ...
        let send_token = token::Send::new(&issuer, Some(bearer.public()), 10, 11);

        let encoded_send_token = send_token.to_vec();
        let decoded_send_token =
            match token::Token::decode(&encoded_send_token).expect("failed to decode token") {
        ...
        assert_eq!(decoded_send_token.issued(), 10);
        assert_eq!(decoded_send_token.expires(), 11);

        let issuer = signing::KeyPair::new();

        let send_token = token::Send::new(&issuer, None, 10, 11);
    ...
    fn subscribe() {
        ...
        let subscribe_token = token::Subscription::new(&issuer, bearer.public(), 10, 11);

        let encoded_subscribe_token = subscribe_token.to_vec();
        let decoded_subscribe_token =
            match token::Token::decode(&encoded_subscribe_token).expect("failed to decode token") {
        ...
        assert_eq!(decoded_subscribe_token.issued(), 10);
        assert_eq!(decoded_subscribe_token.expires(), 11);
    }
}
